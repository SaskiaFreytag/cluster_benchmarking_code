...
sce = create_sce_by_dir("scpipe/subread")
dim(sce)
...
## Save barcodes
#Load barcodes
all_barcodes <- read.csv("barcode_annotation_10X.csv", header=F)
#Match barcodes
...
write.csv(barcode, file="Cleaned_Barcodes_scPipe_SubRead.csv", row.names = F, quote=F)

sce <- calculateQCMetrics(sce, feature_controls = list(mt=rowData(sce)$Mito , rbp=rowData(sce)$ProteinRibo, rbr=rowData(sce)$RNARibo))


## Save sce object for further analysis
save(sce, file="Sce_scPipe_SubRead.RData")
