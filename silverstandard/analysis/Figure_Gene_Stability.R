...
gg1 <- ggplot(res_comp, aes(x=`Method`, y=`ARI_comp`)) + geom_boxplot(aes(color=Method))+ theme_minimal() +  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + annotate("text", label = "*", x = 5, y = 1, size = 8, colour = colors_gg[5])
